...
- Prevented Linux from crashing in the remote-animation retry after a successful player spawn. The animation chain now treats Wii U pointers as unsigned big-endian 32-bit values instead of sign-extending high addresses through `int`, validates every signed offset and host mapping before reading, and reports the exact unavailable link while position synchronization continues.
