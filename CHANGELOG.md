...
- Corrected all 32 `Jugador` actor packs to expose BOTW's verified bow attachment node while retaining `EquipName1`, `EquipName2`, and `EquipName3` for melee, shield, and bow respectively. The sender now preserves the legacy logical held/equipped value instead of transmitting BOTW's inverted raw byte, and change-only wire diagnostics record `WType`, sword, shield, bow, held state, exact resource addresses, requested values, readbacks, and child creation addresses.
