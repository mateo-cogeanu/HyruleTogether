...
- Fixed a Linux `SIGSEGV` introduced while diagnosing remote actor creation. `patch_SpawnActors.asm` aligns its integer fields after two one-byte flags, so its transfer block is 44 bytes including two implicit padding bytes—not 42 bytes. The matching reversed native padding is restored, exact size/offset assertions document and enforce the real PPC ABI, invalid ring-buffer addresses are rejected before dereferencing, and instance storage is read only when an actor is actually queued. Scoped packing still prevents this ABI alignment from leaking into unrelated native types.
